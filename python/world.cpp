...
void export_world()
{
	...
	class_<b2Draw_W, boost::noncopyable>("b2Draw")
		...
		.def_readonly("e_shapeBit", b2Draw::e_shapeBit)
		.def_readonly("e_jointBit", b2Draw::e_jointBit)
		.def_readonly("e_aabbBit", b2Draw::e_aabbBit)
		.def_readonly("e_pairBit", b2Draw::e_pairBit)
		.def_readonly("e_centerOfMassBit", b2Draw::e_centerOfMassBit)
	;
	...
	class_<b2World, b2World_W , boost::noncopyable>("b2World", init<optional<b2Vec2> >())
		...
		.def("GetBodyList", b2World_GetBodyList, with_custodian_and_ward_postcall<0, 1>())
		.def("GetJointList", b2World_GetJointList, with_custodian_and_ward_postcall<0, 1>())
		.def("GetContactList", b2World_GetContactList, with_custodian_and_ward_postcall<0, 1>())
